...
// make sure to use FASTMEM instead of .ccm for Mac based builds...
...
uint8_t font8_data_cached[256][8] FASTMEM;
...
uint8_t font88_data_cached[256][8] FASTMEM;
...
// --------------------------------------------------------------
#elif VGA_SIMPLE_MODE==13 // text mode 50x37
extern const uint8_t font88_data[256][8];
uint8_t font[256][8]  __attribute__ ((section (".ccm")));
// palette attribute
uint32_t palette[1]={0x00007fff}; // white, light grey
...
void graph_line() {
	...
	uint32_t c1=((palette[0]>> 0)&0xffff)*0x00010001;
	uint32_t c2=((palette[0]>>16)&0xffff)*0x00010001;

	uint32_t *dst = (uint32_t*) &draw_buffer[start_x];
	...
	for (int i=0;i<SCREEN_W/2;i++) {  // draw half-screen
		...
		a  = c&128 ? c1&0x0000ffff : c2&0x0000ffff;
		a |= c&64  ? c1&0xffff0000 : c2&0xffff0000;
		*dst++ = a;
		a  = c&32  ? c1&0x0000ffff : c2&0x0000ffff;
		a |= c&16  ? c1&0xffff0000 : c2&0xffff0000;
		*dst++ = a;
		a  = c&8  ? c1&0x0000ffff : c2&0x0000ffff;
		a |= c&4  ? c1&0xffff0000 : c2&0xffff0000;
		*dst++ = a;
		a  = c&2  ? c1&0x0000ffff : c2&0x0000ffff;
		a |= c&1  ? c1&0xffff0000 : c2&0xffff0000;
		*dst++ = a;
