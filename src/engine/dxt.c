...
/* decode a line of palette [c2, c3, ..] from DXT data. */
{
	...
	for (int i=0;i<nb_blocks;i++)
	{
		...
		// -> 000rrrrr_00000000  000rrrrr_00000000 (pourrait etre rrrrr_rrrrr000)
		...
		if (blocks[i].c[0]>blocks[i].c[1]) 
		{
			...
			*/
			...
			c23 = r23 <<9 | g23 <<5 | b23 ; // & (0x1f | 0x1f << 16)) ?
		} 
		else // transparent case
		{
			// add top/bottom (with sat16), should keep low 
			r23=__UQADD16(r01,r01>>16)&0xfe00; // sum on byte 1 as 00rrrrrr_, del low bit 00rrrrr0_00000000
			...
			c23 = r23<<1 | g23>>4 | b23>>9 | 0x80000000;
		}

		*pal32++ =  c23; // set c3 high bit for transp.
	}
}
...
/* decompress 1 line of blocks.
	...
	XXX SPEEDUP : check const blocks / line or other known patterns : 0000 1111 2222 3333 (quad compare ? )
*/
;
...
*/
{
	...
	for (int i=0;i<nb_blocks;i++, palette+=2)
	{
		...
		// fast path if constant 0. TODO : verify if useful.
		if (!pixels)
		{
			*dst++ = blocks[i].c[0];
			*dst++ = blocks[i].c[0];
		...
		for (int pix=0;pix<4;pix++, pixels <<=2) // start from highest bits. Unroll !
		{
			switch ((pixels>>6 & 0x3))
			{
				case 0 : *dst++ = blocks[i].c[0]; break;
				case 1 : *dst++ = blocks[i].c[1]; break;
...
void test_dxt()
{
	const DXTFile testfile= {
		.w=8, // 2 blocks, one transp, one normal
		.h=4,
		.blocks={
			// opaque
			{
			.c = {0b1111100000011111,0b0000011111100000},  // r0b (f81f) / 0g0 (7e0); middle=grey
			.pixels = {0b00011011,0,0,0} // 00 01 10 11
			},
			// transparent
			{
			.c = {0b0000011111100000,0b1111100000011111},  
			.pixels = {0b00011011,0,0,0} 
			},
		}
	};
	uint16_t pixels[8];
	uint16_t palette[4]; // additional palette

	for (int i=0;i<8;i++) pixels[i]=0xaaaa;

	dxt_decode_palette(testfile.blocks, palette, 2);
	printf("palette 0 : %04x %04x\n",palette[0],palette[1]);
	printf("palette 1 : %04x %04x\n",palette[2],palette[3]);
	
	printf("initial : ");for (int i=0;i<8;i++) printf ("%04x ",pixels[i]);printf("\n");
	printf("ligne 0 (trans) : ");

	dxt_decompr_line(testfile.blocks, pixels, 2, palette, 0);
	for (int i=0;i<8;i++) printf ("%04x ",pixels[i]);
	printf("\n");

	printf("ligne 1 : ");
	dxt_decompr_line(testfile.blocks, pixels, 2, palette, 1);
	for (int i=0;i<8;i++) printf ("%04x ",pixels[i]);
	printf("\n");
}
...
void test_cm4()
{
	int16_t data[][4] = {
		{0,0,0,0},
		{0x0001,0x0001,0x1111,0x1111},
		{0x0100,0x0001,0x1111,0x1111},
	};
	int32_t a,b;

	for (int i=0;i<3;i++)
	{
		a=data[i][0]<<16 | data[i][1];
		b=data[i][2]<<16 | data[i][3];
		printf ("__SMUAD : mul/add (%08x, %08x)=%08x\n",a,b,__SMUAD(a,b));
		printf ("__UQADD16 : add16 (%08x, %08x)=%08x\n",a,b,__UQADD16(a,b));
	}
}
