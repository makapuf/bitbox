...
class Blit : 
    ...
    def __len__(self) : 
        "len in words"
        return 1+len(self.data)+(1 if self.fill_c else 0)
...
def encode(name) : 
    ...
    for y in xrange(h) :
        ...
        # line index : 
        if y %16 == 0 : 
            line16_index.append(sum(len(b) for b in blits))
    ...
    if bin_version : 
        ...
        arr.extend([w,h,frame_h,len(line16_index)])
    ...
    */
    ...
    const packbit_rom pb_{name}={{.w = {w}, .h={h}, .frame_h={frame_h}, .idx_len={idx_len}, 
      .data={{
